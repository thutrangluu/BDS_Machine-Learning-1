...
Scaled_Lasso_Loss <- function(y,x,beta,r,lambda){
  ...
  xtx = t(x)%*%x
  yty = t(y)%*%y
  return((t(e)%*%e)/(yty)+lambda*abs(beta)*(xtx/yty)**1/2)
}

## 6.2e
beta_e = c(seq(-0.001,0.001,length.out=100))
...
res1 = lapply(beta_e,FUN=Scaled_Lasso_Loss,y=dfair$airq,r=r_vala,x=X[,2],lambda=0.05)
res2 = lapply(beta_e,FUN=Scaled_Lasso_Loss,y=dfair$airq,r=r_vala,x=X[,2],lambda=0.10)
res3 = lapply(beta_e,FUN=Scaled_Lasso_Loss,y=dfair$airq,r=r_vala,x=X[,2],lambda=0.20)

matplot(beta_e, cbind(res1,res2,res3), col=cols, type="l", lty=1, lwd=2, xlab="beta", ylab="Scaled Lasso Loss")
